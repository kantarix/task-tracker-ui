...
function Column({id, todos, index}: Props) {
    const [searchString, setNewTaskType, setTaskId, setNewTaskNameInput, setNewTaskDescInput] = useBoardStore((state) => [
        state.searchString, 
                        ...
                        >
                            <h2 className="flex justify-between font-bold text-xl p-2">
                                ...
                                <span className="text-gray-500 bg-gray-200 rounded-full px-2 py-2 text-sm font-normal">
                                    {!searchString ? todos.length : todos.filter(todo => todo.name.toLowerCase().includes(searchString.toLowerCase())).length}
                                </span>
                            </h2>
